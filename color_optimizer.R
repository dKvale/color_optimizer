...
#Dorian suggested using round_any <- function(x, accuracy, f = round){ f(x/ accuracy) * accuracy } instead of plyr::round_any to simplify libraries
...
Color_Deets <- function(clrs, df) {
  ...
  fvr <- as.data.frame(farver::decode_colour(clrs, "rgb", "hcl"))
...
Optimize_Color_Palette <- function(clrs, prcnt_transp, min_luminance, max_luminance) {
  ...
  checkingupdated <- Color_Deets(clrs_lum_fixed, df)
...
server <- function(input, output, session) {
  ...
  hex <- renderUI(numIndividuals <- as.integer(input$clrcnt))
  ...
  output$palettepicker <- renderUI({
    if(reactive_values$cnt>1 & reactive_values$cnt<11){
      ...
      # selectInput("pal", label = "Choose a palette",
 ...
 output$showThePalette <- renderTable(HTML_DF(allhexs()),
